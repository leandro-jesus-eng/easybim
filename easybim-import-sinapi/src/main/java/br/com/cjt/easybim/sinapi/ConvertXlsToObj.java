...
public class ConvertXlsToObj {
	...
	private void parseComposicoes() {
		...
		for (int row = (rowHeader + 1); row < range.getRowCount(); row++) {
			...
			// se a coluna codigoItem(12) for vazia então é uma composicao PAI
			if (range.get(row, codigoItemColumnIndex).getStringValue().isEmpty()) {
				...
				// verificar se é uma composicao ou insumo
	...
	public TabelaCustosIndices parse(String file) {
		...
		try {
			...
			System.out.println("Número de composicoes no MAP "+ composicaoMap.size() );
			System.out.println("Número de composicoes no tabela "+ tabelaCustosIndices.getComposicoes().size() );
			System.out.println("Número de insumos no tabela "+ tabelaCustosIndices.getInsumos().size() );
